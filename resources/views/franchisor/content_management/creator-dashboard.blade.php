            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        action="{{ route('franchisor.creator-dashboard.update', Auth::guard('franchisor')->id()) }}">
                        @csrf
                        @method('PUT')

                            <div class="form-group mb-2 {{ $errors->has('header') ? 'has-error' : '' }} ">
                                <label for="header">Main Title</label>
                                <input type="text" class="form-control" id="header" value="{{ old('header', $franchisor->header) }}" name="header"
                            ...
                            <div class="form-group mb-2 {{ $errors->has('title') ? 'has-error' : '' }}">
                                ...
                                <input type="text" class="form-control" id="title" name="title" 
                                    placeholder="Enter Title" value="{{ old('title', $franchisor->title) }}">
                                @error('title')
                            ...
                            <div class="form-group mb-2 {{ $errors->has('description') ? 'has-error' : '' }}">
                                ...
                                <textarea rows="4" cols="4" class="form-control" id="description" name="description"
                                    placeholder="Enter Description">{{ old('description', $franchisor->description) }}</textarea>
                                @error('description')
