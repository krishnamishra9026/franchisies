...
<div class="upgrades" id="content">
    ...
    <form method="POST" action="{{ route('creator.upgarde-save') }}">
    @csrf
    <div class="container">
            ...
            <div class="row">
                <div class="col-sm-3">
                    <div class="featured-checkbox">
                        ...
                        @if(!in_array($badge->id, $badge_ids??[])) <input type="checkbox" value="1" @if(in_array($badge->id, $badge_ids??[])) checked @endif name="badges[checked][{{$badge->id}}]" /> @endif <span style="background-color:{{ $badge->color ?? "#f07238" }}">{{ $badge->name }}</span>
                        @if(in_array($badge->id, $badge_ids??[])) <h6>Activated</h6> @endif
                    </div>
